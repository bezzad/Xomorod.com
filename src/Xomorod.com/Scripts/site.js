...
jQuery(document).ready(function () {
    /* load google analystics script by jquery */
    setTimeout(function () {
    ...
    $("#MobileVersion").html(function () {
        return isMobile ? window.localization.mobile : window.localization.desktop;
    });
    ...
    $.ajaxSetup({
        error: function (xhr, status, error) {
            try {
                ...
                if (err.ExceptionMessage !== undefined && err.ExceptionMessage !== null && err.ExceptionMessage !== "")
        ...
        complete: function () {
            //toastr.info(window.localization.requestCompleted);
        }
    });
...
function getUrlCulture() {
    ...
    if (path.indexOf("/") !== 0 || path.length < 3 || path.substring(1).indexOf("/") !== 2) return "en";
...
function getAsync(url, params) {
    if (url === null) {
        toastr.warning(window.localization.entryUrlIsEmpty, window.localization.warning, { timeOut: 15000 });
        return;
    }
    toastr.info(window.localization.pleaseWait, "", { timeOut: 3000 });
